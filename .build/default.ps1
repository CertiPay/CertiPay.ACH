...
Task Test -depends Build, Install-NUnitRunner {
	exec { . $NUnit "$SolutionRoot\CertiPay.ACH.Tests\bin\$Configuration\CertiPay.ACH.Tests.dll" }
}

Task Package -depends Update-AssemblyInfoFiles, Build {
	exec { . $NuGet pack "$SolutionRoot\CertiPay.ACH\CertiPay.ACH.nuspec" -Properties Configuration=$Configuration -OutputDirectory "$SolutionRoot" -Version "$Version" }
}
...
Task Install-MSBuild {
	...
	{
		Write-Output "MSBuild not found, Installing..."
